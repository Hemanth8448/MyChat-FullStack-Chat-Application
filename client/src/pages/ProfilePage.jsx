...
const ProfilePage = () => {
    ...
    const handleSubmit = async (e)=>{
        e.preventDefault();
        if(!selectedImg){
            console.log("not profile selected")
            ...
            <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
                <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
                    ...
                    <textarea onChange={(e)=>setBio(e.target.value)} value={bio}
                              rows={4} placeholder='Write profile Bio'
                              className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500' required/>
                    <button
                            type='submit'
                ...
                <img className={`max-w-44 aspect-squre rounded-full mx-10 max-sm:mt-10 ${selectedImg && 'w-[35px] aspect-[1/1] rounded-full'}`} src={authUser.profilePic || assets.logo_icon} alt=""/>
            </div>
