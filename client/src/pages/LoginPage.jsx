...
const LoginPage = () => {
    ...
    return(
        <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
            {/*------left image-----*/}
            <img src={assets.logo_big} alt='' className='w-[min(50vw,400px)]'/>
            {/*----right------*/}
                ...
                )}
                {
                    currState==='Sign up' && isDataSubmitted && (
                        <textarea onChange={(e)=>setBio(e.target.value)}
                                  value={bio} rows={4}
                                  placeholder='Provide a Short Bio' className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-indigo-500' required>
